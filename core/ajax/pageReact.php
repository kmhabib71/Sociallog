...
//  $reactExist=$getFromP->reactExistingCheck( $page_id, $tweet_id, $user_id); 
...
//   
//// if($rex >0 ){
     $getFromP->unReactExist($page_id, $user_id, $tweet_id, $get_id);
      if($sl < 1 ){ 


    $getFromP->addReact($page_id, $user_id, $tweet_id, $get_id,$react_type);
    echo 'hello';
 }
...
if(isset($_POST['unHappyReact']) && !empty($_POST['unHappyReact'])){
    ...
    $get_id = $_POST['user_id'];
    $page_id = $_POST['page_id'];
    $getFromP->unReact($page_id, $user_id, $tweet_id, $get_id, $react_type);
